...
export default function AttestationAffiliation() {
    ...
    const sendRequest = async () => {
      try {
        const response = await fetch(
          `http://localhost:5000/api/attestationAffiliation/`
        );
                            ...
                            onClick={async (event) => {
                              try {
                                let response = await fetch(
                                  `http://localhost:5000/api/attestationAffiliation/${row._id}`,
                                  {
                                    method: "PATCH",
                                    headers: {
                              ...
                              try {
                                let response = await fetch("http://localhost:5000/api/notification/addNotification", {
                                  ...
                                  body: JSON.stringify({
                                    sujet: "Demande attestation d'affiliation",
                                    message: "Votre demande d'attestation d'affiliation est bien pris en charge",
