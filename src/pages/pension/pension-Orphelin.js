...
import ModalGalerie from "../../components/Modal/modalGaleriePensionOrphelin";
...
export default function PensionOrphelin() {
    ...
    const sendRequest = async () => {
      try {
        const response = await fetch(
          `http://localhost:5000/api/pensionOrphelin/`
        );
                            ...
                            onClick={async (event) => {
                              try {
                                let response = await fetch(
                                  `http://localhost:5000/api/pensionOrphelin/${row._id}`,
                                  {
                                    method: "PATCH",
                                    headers: {
